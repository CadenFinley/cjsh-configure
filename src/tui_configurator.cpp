...
static const std::vector<std::string> main_menu = {
    "1) Edit Interactive (~/.cjshrc)",
    "2) Edit Login Profile (~/.cjprofile)",
    "3) Manage Themes",
    "4) Manage Plugins",
    "5) Exit"
};
...
static void showMenu(const std::string& title, const std::vector<std::string>& menu) {
    int choice = 0;
    while (true) {
        clear();
        {
        ...
        int c = getch();
        switch(c) {
            case KEY_UP:    choice=(choice+menu.size()-1)%menu.size(); break;
            case KEY_DOWN:  choice=(choice+1)%menu.size(); break;
            case '\n':
...
static void add_alias_menu(const std::string& path) {
    clear();
    mvprintw(0,0,"Alias name: "); echo(); curs_set(1);
    char name[128]; getnstr(name,127);
    mvprintw(1,0,"Command  : "); 
    char cmd[1024]; getnstr(cmd,1023);
    noecho(); curs_set(0);
    ...
    mvprintw(3,0,"Alias added. Press any key..."); getch();
}

static void add_startup_command_menu(const std::string& path) {
    clear();
    mvprintw(0,0,"Startup command: "); echo(); curs_set(1);
    char cmd[1024]; getnstr(cmd,1023);
    noecho(); curs_set(0);
...
static void add_env_var_menu(const std::string& path) {
    clear();
    mvprintw(0,0,"Variable name : "); echo(); curs_set(1);
    char var[128]; getnstr(var,127);
    mvprintw(1,0,"Variable value: ");
    char val[1024]; getnstr(val,1023);
    noecho(); curs_set(0);
    ...
    mvprintw(3,0,"Export added. Press any key..."); getch();
}

static void add_theme_menu(const std::string& path) {
    clear();
    mvprintw(0,0,"Theme name: "); echo(); curs_set(1);
    char theme[128]; getnstr(theme,127);
    noecho(); curs_set(0);
    ...
    mvprintw(2,0,"Theme set. Press any key..."); getch();
}

static void add_plugin_menu(const std::string& path) {
    clear();
    mvprintw(0,0,"Plugin name: "); echo(); curs_set(1);
    char plugin[128]; getnstr(plugin,127);
    noecho(); curs_set(0);

    std::ofstream ofs(path, std::ios::app);
    ofs << "plugin " << plugin << " enable\n";
    mvprintw(2,0,"Plugin added. Press any key..."); getch();
}

static void add_startup_arg(const std::string& path) {
    clear();
    mvprintw(0,0,"Startup argument: "); echo(); curs_set(1);
    char arg[1024]; getnstr(arg,1023);
    noecho(); curs_set(0);
...
static void configureFile(const std::string& path,
    ...
    { std::ofstream _c(path, std::ios::app); }
    int choice = 0;
    while (true) {
        clear();
        {
        ...
        int c = getch();
        switch(c) {
            case KEY_UP:    choice=(choice+edit_items.size()-1)%edit_items.size(); break;
            case KEY_DOWN:  choice=(choice+1)%edit_items.size(); break;
            case '\n': {
                int exit_idx   = edit_items.size() - 1;
                int wipe_idx   = edit_items.size() - 2;
                int remove_idx = edit_items.size() - 3;
                if (choice == exit_idx) return;
                if (choice == wipe_idx) {
                    std::ofstream ofs(path);
                    clear();
                ...
                if (choice == remove_idx) {
                    ...
                    if (path.find(".cjshrc") != std::string::npos) {
                        switch (choice) {
                            case 0: add_alias_menu(path);            break;
                            case 1: add_startup_command_menu(path); break;
                            case 2: add_env_var_menu(path);         break;
                            case 3: add_theme_menu(path);           break;
                            case 4: add_plugin_menu(path);          break;
                        }
                    }
                        ...
                        switch (choice) {
                            ...
                            case 1: add_startup_command_menu(path); break;
                            case 2: add_env_var_menu(path);         break;
                            case 3: add_startup_arg(path);          break;
                        }
                    }
                }
            } break;
...
void Configurator::run() {
    ...
    while (true) {
        ...
        int c = getch();
        switch(c) {
            case KEY_UP:    choice=(choice+main_menu.size()-1)%main_menu.size(); break;
            case KEY_DOWN:  choice=(choice+1)%main_menu.size(); break;
            case '\n':
                endwin();
                if      (choice==0) { configureFile(rc,    edit_items_cjshrc); }
                else if (choice==1) { configureFile(profile,edit_items_cjprofile); }
                else if (choice==2) { showMenu("Manage Themes",  theme_menu); }
                else if (choice==3) { showMenu("Manage Plugins", plugin_menu); }
